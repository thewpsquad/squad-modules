...
<div class="ext-copy-content" id="squad_ext_copy_content" style="display: none;">
		...
		<div class="fieldset-container">
			...
			<div class="align-left">
				<label>
					...
					<?php $disq_current_site = get_current_blog_id(); ?>
					<?php $disq_site_is_multi = is_multisite() !== true ? ' disabled' : ''; ?>

					<select name="copied-post-target-site" <?php echo esc_attr( $disq_site_is_multi ); ?>>

						<option value="<?php echo esc_attr( $disq_current_site ); ?>" data-type="dynamic">
							<?php esc_html_e( 'this site', 'squad-modules-for-divi' ); ?>
						</option>

						<?php if ( function_exists( 'is_multisite' ) && is_multisite() ) : ?>

							<?php $disq_current_sites = $wpdb->get_results( "SELECT blog_id FROM $wpdb->blogs ORDER BY blog_id" ); ?>

							<?php foreach ( $disq_current_sites as $disq_site ) : ?>
								<?php if ( $disq_current_site !== (int) $disq_site->blog_id ) : ?>

									<option value="<?php echo esc_attr( $disq_site->blog_id ); ?>" data-type="dynamic">
										<?php echo esc_html( get_blog_option( $disq_site->blog_id, 'blogname' ) ); ?>
									</option>
	...
	<button class="disq-admin-button fill-button" data-btn="copy-quick">
