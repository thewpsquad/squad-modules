...
abstract class Migration {
	 ...
	 * @return mixed
	 */
	protected static function migrate_field_names( $fields, $module_slug, $version ) {
	...
	public static function get_migrations( $module_version ) {
		...
		foreach ( self::$migrations as $version => $migration ) {
			...
			if ( is_string( $migration ) ) {
				$migration = ( __NAMESPACE__ . '\\Migration\\' . $migration );
	...
	public static function maybe_override_shortcode_attributes( $attrs, $unprocessed_attrs, $module_slug, $module_address, $content = '', $maybe_global_presets_migration = false ) {
		...
		foreach ( $migrations as $migration ) {
			...
			// It needs for IconManager's unit tests when it is necessary to test the migration of module posts attributes
			// and migration of global presets within the same test session
			// (because a migration fields array is depending on self::$maybe_global_presets_migration variable ).
	...
	/**
	 * This could have been written as abstract, but its not as common to be expected to be implemented by every migration
	 *
	 * @param string $module_slug Internal system name for the module type.
	 * @param array  $attrs       Shortcode attributes.
	 * @param mixed  $content     Text/HTML content within the current module.
